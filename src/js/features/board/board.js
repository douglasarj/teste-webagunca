...
async function loadCardsFromDatabase(boardId) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.CARDS_BY_BOARD(boardId)), {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function moveCardToNewList(cardId, sourceListId, targetListId) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.CARD_BY_ID(cardId)), {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
