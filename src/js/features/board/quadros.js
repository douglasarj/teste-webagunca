...
async function loadBoards() {
    ...
    try {
        const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.BOARDS), {
            method: 'GET',
            headers: {
...
function showDeleteBoardConfirmation(board) {
    ...
    confirmBtn.addEventListener('click', async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.BOARD_BY_ID(board._id)), {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
...
function showCreateBoardModal() {
    ...
    createBtn.addEventListener('click', () => {
        ...
        fetch(getApiUrl(API_CONFIG.ENDPOINTS.BOARDS), {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}` 
